...
if __name__ == '__main__':
    files = os.listdir('selected_patches/')
    thresholds_list = []
    for one_file in files:
        filename = 'selected_patches/' + one_file
        dot = filename.index('.')
        ...
        savename = prefix + str(6) + extension
        print(prefix)
        kThresholds = otsu.calculate_k_thresholds(7)
        print(kThresholds)
        thresholds_list.append(kThresholds)
        # crushed3 = otsu.apply_thresholds_to_image(kThresholds[:2])
        # imager.write(savename, crushed3_)
        
    # print(thresholds_list)
    temp_thres = [0, 0, 0, 0, 0, 0, 0]
    for one in thresholds_list:
        for i in range(7):
            temp_thres[i] += one[i]

    for i in range(7):
        temp_thres[i] = temp_thres[i] / 5
